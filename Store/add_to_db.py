...
class Database:
    ...
    def get_items(self, *args, **kwargs):
        ...
        self.vender_phone = self.vendor_phone_e.get()
        ...
        if self.name ==''or self.stock == '' or self.cp == '' or self.sp =='':
            tkinter.messagebox.showinfo("Error","Por favor Preencha todos Campos.")
        else:
            sql = "INSERT INTO inventory (name, stock, cp, sp, totalcp, totalsp, assumed_profit, vendor, vendor_phoneno) VALUES(?,?,?,?,?,?,?,?,?)"
            c.execute(sql,(self.name, self.stock, self.cp, self.sp, self.totalcp, self.totalsp, self.assumed_profit, self.vendor, self.vender_phone))
            conn.commit()

            # textbox insert
            self.tBox.insert(END,"\n\nInserted"+ str(self.name)+"into the database."+ str(self.id_e.get()))
            tkinter.messagebox.showinfo("Sucess","Registado com Sucesso")
